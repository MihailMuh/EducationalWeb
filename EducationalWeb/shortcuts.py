...
async def afilter(klass: Type[Model], *args, **kwargs) -> QuerySet:
    return await database_sync_to_async(klass.objects.filter)(*args, **kwargs)
...
async def aget_query(query: QuerySet, *args, **kwargs):
    try:
        return await database_sync_to_async(query.get)(*args, **kwargs)
    except query.model.DoesNotExist:
        return None


async def aget_or_create(klass: Type[Model], **kwargs) -> tuple[Any, bool]:
    return await database_sync_to_async(klass.objects.get_or_create)(**kwargs)
...
aget_object_or_404 = database_sync_to_async(get_object_or_404)
